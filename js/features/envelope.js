...
function renderInboxList() {
    ...
    list.innerHTML = envelopeData.inbox.slice().reverse().map(letter => {
        ...
        // chat 新增：梦角主动来信标识
        const fromPartner = letter.fromPartner;
        const senderLabel = fromPartner ? ((typeof settings !== 'undefined' && settings.partnerName) || '对方') + '的来信' : '收到';
        return `
...
window.viewEnvLetter = function(section, id) {
    ...
    else if (section === 'inbox' && letter.fromPartner) {
