...
// ============================================================
// 整合：将 chat 版本的 renderComboMenu 与 zmilk 版本合并
// 保留 zmilk 的拍一拍管理（带删除按钮），同时加入 chat 的“语音”标签页
// ============================================================
...
function showEmojiTab() {
    ...
    if (groups.length === 0) {
        // 没有分组，直接渲染全部
        _renderStickerGrid(area, enabledStickers);
        return;
    }
...
// 辅助函数：渲染表情网格
function _renderStickerGrid(container, stickers) {
    container.innerHTML = '';
...
// ===== showPokeTab 保留 zmilk 版本（带删除和清空功能） =====
...
// ===== showVoiceTab 来自 chat 版本（新增语音功能） =====
    ...
    voices.forEach((voice, idx) => {
        ...
        item.innerHTML = `
            ...
            <div style="flex:1;min-width:0;">
                <div style="font-size:13px;font-weight:600;color:var(--text-primary);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${_esc(voice.text || '未命名语音')}</div>
            </div>
