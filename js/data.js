(function () {
    ...
    // ===== 采用 chat 版本的 applyStats（支持 navigator.storage.estimate） =====
    ...
    // ===== 采用 chat 版本的 updateStats（更精确的统计逻辑） =====
...
// ===== 保留 zmilk 的 updateStorageUsageBar，但内部调用 updateStats =====
            ...
            if (window.localforage && window.APP_PREFIX) {
                localforage.keys().then(function(keys) {
                    ...
                    Promise.all(promises).then(function(sizes) {
                        ...
                        var text = document.getElementById('dm-storage-total');
                        if (bar) {
                            var maxBytes = 5 * 1024 * 1024;
                            var pct = Math.min(total / maxBytes * 100, 100).toFixed(1);
                            bar.style.width = pct + '%';
                            if (parseFloat(pct) > 80) bar.style.background = 'linear-gradient(90deg,#FF3B30,#CC0000)';
                            else if (parseFloat(pct) > 50) bar.style.background = 'linear-gradient(90deg,#FF9F0A,#E07000)';
                            else bar.style.background = 'linear-gradient(90deg,var(--accent-color),rgba(var(--accent-color-rgb),0.6))';
                        }
                        if (text) {
                            var fmt = function(b) { return b<1024 ? b+' B' : b<1048576 ? (b/1024).toFixed(1)+' KB' : (b/1048576).toFixed(2)+' MB'; };
                            var maxBytes = 5 * 1024 * 1024;
                            var pct = Math.min(total / maxBytes * 100, 100).toFixed(1);
                            text.textContent = fmt(total) + ' / ~5 MB (' + pct + '%)';
                        }
                    });
                }).catch(function() {});
            }
...
// ===== 保留 zmilk 的 handleNotifToggle（已存在） =====
...
// ===== 保留 zmilk 的 DOMContentLoaded 初始化（包含通知状态同步） =====
...
// ===== 保留 zmilk 的 showModal 覆盖（用于数据弹窗打开时更新统计） =====
(function() {
