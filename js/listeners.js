...
if (envelopeReplyMaxSentencesSlider) {
    ...
    if (envelopeHeaderBtn) {
        envelopeHeaderBtn.addEventListener('click', () => {
            hideModal(DOMElements.advancedModal.modal);
            if (typeof loadEnvelopeData === 'function' && typeof checkEnvelopeStatus === 'function') {
                loadEnvelopeData().then(() => {
                    checkEnvelopeStatus();
                    currentEnvTab = 'outbox';
                    document.getElementById('env-tab-outbox').classList.add('active');
                    document.getElementById('env-tab-inbox').classList.remove('active');
                ...
                showNotification('信封功能未加载', 'error');
            }
        });
    }
